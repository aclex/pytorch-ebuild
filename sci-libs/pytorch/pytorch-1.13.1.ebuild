...
SLOT="0/${PV}"
KEYWORDS="~amd64 ~x86"
IUSE="asan atlas caffe2 cuda eigen +fbgemm ffmpeg gflags glog +gloo leveldb lmdb mkl +mkldnn mpi namedtensor +nnpack numa +numpy +observers openblas opencl opencv +openmp +python qnnpack redis rocm static tbb test tools xnnpack zeromq"
...
DEPEND="
	...
	>=sci-libs/onnx-1.13[${PYTHON_USEDEP}]
...
PATCHES=(
	"${FILESDIR}/1.13/Use-FHS-compliant-paths-from-GNUInstallDirs-module.patch"
	...
	"${FILESDIR}/1.13/Fix-ROCm-paths-in-LoadHIP.cmake.patch"
	...
	"${FILESDIR}/1.13/Use-public-API-for-IsGoogleLoggingInitialized-of-glog.patch"
	"${FILESDIR}/1.13/Don-t-use-NEON_2_SSE-header.patch"
)
	...
	if use python; then
		...
		fix_caffe_convert_utils() {
			...
			if use caffe2; then
				...
				find "${BUILD_DIR}/caffe2/proto" -name "*_pb2.py" -print0 | xargs -0 -n1 cp -v {} "${D}/${PYTHON_SITEDIR}/caffe2/proto" || die
			else
				rm -v "${D}/usr/bin/convert-caffe2-to-onnx" || die
				rm -v "${D}/usr/bin/convert-onnx-to-caffe2" || die
			fi
